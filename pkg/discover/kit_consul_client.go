...
func NewKitConsulClient(consulHost string, consulPort uint16) (DiscoveryClient, error) {
...
func (consulClient *KitConsulClient) Register(serviceName, instanceId, instanceHost string, instancePort uint16, meta map[string]string, logger *log.Logger) bool {
	...
	if err != nil {
		log.Printf("Register Service Error! Cased by: %s\n", err)
		return false
	}
...
func (consulClient *KitConsulClient) DeRegister(instanceId string, logger *log.Logger) bool {
	...
	if err != nil {
		logger.Printf("Deregister Service Error! Cased by: %s\n", err)
		return false
	}
...
func (consulClient *KitConsulClient) DiscoverServices(serviceName string, logger *log.Logger) []interface{} {
	...
	if err != nil {
		consulClient.instancesMap.Store(serviceName, []interface{}{})
		logger.Printf("Discover Service Error! Cased by: %s\n", err)
		return nil
	}
